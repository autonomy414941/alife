...
export const METRICS_CSV_COLUMNS = [
  ...
  'phenotype_effective_richness',
  'phenotype_mean_pairwise_distance',
  'phenotype_occupied_niches',
  'phenotype_species_per_occupied_niche',
  ...
  'policy_agents_harvest_fraction',
  'policy_agents_movement_fraction',
  'policy_agents_reproduction_fraction',
  'policy_decisions_gated_fraction',
  'policy_harvest_decisions_guided_fraction',
  'policy_movement_decisions_gated_fraction',
  'policy_reproduction_decisions_gated_fraction',
  'policy_movement_decisions',
  'policy_movement_gated_decisions',
  'policy_movement_energy_reserve_active_decisions',
  'policy_movement_recent_harvest_active_decisions',
  'policy_movement_blocked_by_energy_reserve',
  'policy_movement_blocked_by_recent_harvest',
  'policy_movement_energy_reserve_near_threshold',
  'policy_movement_recent_harvest_near_threshold',
  'policy_reproduction_decisions',
  'policy_reproduction_gated_decisions',
  'policy_reproduction_harvest_threshold_active_decisions',
...
export const DISTURBANCE_GRID_STUDY_CSV_COLUMNS = [
  ...
  'seed_blocks',
  'hypothesis_support_block_fraction',
  'path_dependence_positive_block_fraction',
  'relapse_event_reduction_positive_block_fraction',
  'resilience_delta_positive_fraction_block_mean',
  'resilience_delta_positive_fraction_block_min',
  'resilience_delta_positive_fraction_block_max',
  'memory_delta_positive_fraction_block_mean',
  'memory_delta_positive_fraction_block_min',
  'memory_delta_positive_fraction_block_max',
  'relapse_event_reduction_positive_fraction_block_mean',
  'relapse_event_reduction_positive_fraction_block_min',
  'relapse_event_reduction_positive_fraction_block_max',
  'turnover_spike_reduction_positive_fraction_block_mean',
  'turnover_spike_reduction_positive_fraction_block_min',
  'turnover_spike_reduction_positive_fraction_block_max',
  'path_dependence_gain_positive_fraction_block_mean',
  'path_dependence_gain_positive_fraction_block_min',
  'path_dependence_gain_positive_fraction_block_max',
  'latest_recovery_lag_reduction_positive_fraction_block_mean',
  'latest_recovery_lag_reduction_positive_fraction_block_min',
  'latest_recovery_lag_reduction_positive_fraction_block_max',
...
export function cladeActivityCladogenesisHorizonSweepToJson(
  exportData: CladeActivityCladogenesisHorizonSweepExport
): string {
  return `${JSON.stringify(exportData, null, 2)}\n`;
}

export function cladeSpeciesActivityCouplingToJson(exportData: CladeSpeciesActivityCouplingExport): string {
  return `${JSON.stringify(exportData, null, 2)}\n`;
}
...
export function cladeActivityRelabelNullCladeInteractionCouplingSweepToJson(
  exportData: CladeActivityRelabelNullCladeInteractionCouplingSweepExport
): string {
  return `${JSON.stringify(exportData, null, 2)}\n`;
}
  ...
  for (let i = 0; i < summaries.length; i += 1) {
    ...
    rows.push(
      toCsvRow([
        ...
        summary.phenotypeDiversity?.meanPairwiseDistance ?? '',
        summary.phenotypeDiversity?.occupiedNiches ?? '',
        summary.phenotypeDiversity?.speciesPerOccupiedNiche ?? '',
        summary.policySensitivePhenotypeDiversity?.effectiveRichness ?? '',
        summary.policySensitivePhenotypeDiversity?.meanPairwiseDistance ?? '',
        summary.policySensitivePhenotypeDiversity?.occupiedNiches ?? '',
        summary.policySensitivePhenotypeDiversity?.speciesPerOccupiedNiche ?? '',
        ...
        summary.policyObservability?.activation.harvestPolicyAgentFraction ?? '',
        ...
        summary.policyObservability?.activation.harvestDecisionGuidedFraction ?? '',
        ...
        summary.policyObservability?.movement.decisions ?? '',
        summary.policyObservability?.movement.gatedDecisions ?? '',
        summary.policyObservability?.movement.energyReservePolicyActiveDecisions ?? '',
        summary.policyObservability?.movement.recentHarvestPolicyActiveDecisions ?? '',
        summary.policyObservability?.movement.blockedByEnergyReserve ?? '',
        summary.policyObservability?.movement.blockedByRecentHarvest ?? '',
        summary.policyObservability?.movement.energyReserveNearThreshold ?? '',
        summary.policyObservability?.movement.recentHarvestNearThreshold ?? '',
        summary.policyObservability?.reproduction.decisions ?? '',
        summary.policyObservability?.reproduction.gatedDecisions ?? '',
        summary.policyObservability?.reproduction.harvestThresholdPolicyActiveDecisions ?? '',
        summary.policyObservability?.reproduction.suppressedByHarvestThreshold ?? '',
        summary.policyObservability?.reproduction.harvestThresholdNearThreshold ?? '',
  ...
  for (const cell of exportData.cells) {
    rows.push(
      toCsvRow([
        ...
        cell.reproducibility.blocks,
        cell.reproducibility.hypothesisSupportFraction,
        cell.reproducibility.pathDependenceGainPositiveBlockFraction,
        cell.reproducibility.relapseEventReductionPositiveBlockFraction,
        cell.reproducibility.resilienceStabilityPositiveFraction.mean,
        cell.reproducibility.resilienceStabilityPositiveFraction.min,
        cell.reproducibility.resilienceStabilityPositiveFraction.max,
        cell.reproducibility.memoryStabilityPositiveFraction.mean,
        cell.reproducibility.memoryStabilityPositiveFraction.min,
        cell.reproducibility.memoryStabilityPositiveFraction.max,
        cell.reproducibility.relapseEventReductionPositiveFraction.mean,
        cell.reproducibility.relapseEventReductionPositiveFraction.min,
        cell.reproducibility.relapseEventReductionPositiveFraction.max,
        cell.reproducibility.turnoverSpikeReductionPositiveFraction.mean,
        cell.reproducibility.turnoverSpikeReductionPositiveFraction.min,
        cell.reproducibility.turnoverSpikeReductionPositiveFraction.max,
        cell.reproducibility.pathDependenceGainPositiveFraction.mean,
        cell.reproducibility.pathDependenceGainPositiveFraction.min,
        cell.reproducibility.pathDependenceGainPositiveFraction.max,
        cell.reproducibility.latestRecoveryLagReductionPositiveFraction.mean,
        cell.reproducibility.latestRecoveryLagReductionPositiveFraction.min,
        cell.reproducibility.latestRecoveryLagReductionPositiveFraction.max,
